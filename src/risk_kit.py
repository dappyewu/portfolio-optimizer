...
def drawdown(return_series: pd.Series):
  ...
  prvpeaks = wealth_index.cummax()
  dd = (wealth_index - prvpeaks)/prvpeaks
  dddate = dd.idxmin()
  drawdown = dd.min()
  return pd.DataFrame({'Wealth': wealth_index, 'Peaks': prvpeaks, 'Drawdown': dd})


def drawdowninfo(return_series: pd.Series):
  wealth_index = 1000*(1+return_series).cumprod()
  prvpeaks = wealth_index.cummax()
  dd = (wealth_index - prvpeaks)/prvpeaks
  dddate = dd.idxmin()
  drawdown = dd.min()
  #return {'Drawdown Date was': dddate, 'Drawdown %': drawdown}
  return f"Drawdown date was  {dddate},and the drawdown percentage was {round(100 * drawdown,2)}%."
...
def get_ind_returns():
  ...
  fileview = pd.read_csv('ind30_m_vw_rets.csv',  header =0, index_col = 0 , parse_dates=True)
  fileview.index = pd.to_datetime(fileview.index, format="%Y%m").to_period('M')
  fileview = fileview/100
  fileview.columns = fileview.columns.str.strip()
  return fileview

def get_ind_size():

  fileview = pd.read_csv('ind30_m_size.csv',  header =0, index_col = 0 , parse_dates=True)
  fileview.index = pd.to_datetime(fileview.index, format="%Y%m").to_period('M')
  fileview.columns = fileview.columns.str.strip()
  return fileview
...
def get_ind_firms():
  ...
  fileview = pd.read_csv('ind30_m_nfirms.csv',  header =0, index_col = 0 , parse_dates=True)
  fileview.index = pd.to_datetime(fileview.index, format="%Y%m").to_period('M')
  fileview.columns = fileview.columns.str.strip()
  return fileview
...
# Lazy / defensive: don't take down the whole module if FRED is unreachable
# or the key is missing. Callers that need a live rate should fetch it
# themselves; this module-level constant is only used as a fallback default
# for rolling_sharpe_ratio's signature.
load_dotenv()
FRED_API_KEY = os.getenv('FRED_API_KEY')
risk_free_rate = 0.04  # sensible fallback if FRED call below fails
try:
    if FRED_API_KEY:
        fred = Fred(api_key=FRED_API_KEY)
        ten_year_treasury_rate = fred.get_series_latest_release('GS10') / 100
        risk_free_rate = float(ten_year_treasury_rate.iloc[-1])
except Exception as _fred_err:
    print(f"[risk_kit] Warning: FRED lookup failed at import ({_fred_err!s}); using fallback risk_free_rate=0.04")
...
def rolling_sharpe_ratio(
    ...
    """Compute a *rolling* (moving-window) Sharpe Ratio.

    Parameters
    ----------
    r : pd.Series or pd.DataFrame
        Periodic returns (e.g. daily returns). If DataFrame, Sharpe is
        computed per-column.
    window : int
        Rolling lookback window in periods (e.g. 63 ≈ 3 months of trading days).
    riskfree_rate : float
        Annual risk-free rate (e.g. 0.04 for 4%).
    periods_per_year : int
        Number of return periods per year (252 for daily, 12 for monthly).
    min_periods : int | None
        Minimum observations in window required to compute Sharpe. Defaults to
        `window`.

    Returns
    -------
    pd.Series or pd.DataFrame
        Rolling Sharpe Ratio series aligned to input index.
    """
    ...
    roll_std = r.rolling(window=window, min_periods=min_periods).std(ddof=0)

    # Annualise the rolling Sharpe: SR = sqrt(ppy) * mean(excess) / std
